...
import CometChatOutgoingCall from '../../cometchat-pro-react-native-ui-kit/CometChatWorkspace/src/components/Calls/CometChatOutgoingCall';
import CometChatIncomingCall from '../../cometchat-pro-react-native-ui-kit/CometChatWorkspace/src/components/Calls/CometChatIncomingCall';
import CometChatParentList from './CometChatParentList';
import { CometChatManager } from '../../cometchat-pro-react-native-ui-kit/CometChatWorkspace/src/utils/controller';
import CometChatImageViewer from '../../cometchat-pro-react-native-ui-kit/CometChatWorkspace/src/components/Messages/CometChatImageViewer';
...
import {blockUnblockUserNotify_handle} from '../../utils/notification/notificationApi'
class ParentOfChild extends React.Component {
  loggedInUser = null;
    ...
    CometChatManager.blockUsers(usersList)
      .then((response) => {
        if (response) {
          ...
          blockUnblockUserNotify_handle(this.loggedInUser.uid, true)
        } else {
    ...
    CometChatManager.unblockUsers(usersList)
      .then((response) => {
        if (response) {
          ...
          if (this.loggedInUser.role == 'user')
          blockUnblockUserNotify_handle(this.loggedInUser.uid, false)
        } else {
  ...
  audioCall = () => {
    try {
      ...
      CometChatManager.call(receiverId, receiverType, CometChat.CALL_TYPE.AUDIO)
        ...
        .catch((err) => {console.log('call err', err)});
    } catch (error) {
      logger(error);
    }
  };
      ...
      <CometChatContextProvider ref={(el) => (this.contextProviderRef = el)}>
        <View style={{ backgroundColor: 'white' }}>
          <CometChatParentList
            ...
            user_image_path = {this.props.route?.params?.user_image_path}
          />
          {imageView}
          {this.state.isActive ? 
          <>
